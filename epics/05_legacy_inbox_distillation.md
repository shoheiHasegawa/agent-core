...
## バックログ (Backlog)
- [ ] `agent-core/workspaces/Proj_Legacy_Distillation/raw_data/` に退避された全ファイル（約80件）のインベントリリストを作成し、AIによる一括仕分け（Delete/Distill/Absorb）の提案レポートを出力する。
- [ ] 提案レポートを社長がレビュー後、Agentが一括でフォーマット変換・移動処理を実行する。
- [ ] 処理完了後、`raw_data/` ディレクトリおよび本ワークスペースをTeardownする。
