...
from infrastructure.task_management.worklog_repository import WorklogRepository
from infrastructure.db.models import SessionLocal
...
class TaskManagementFactory:
    ...
    @staticmethod
    def create_task_repository(session: Session = None) -> TaskRepository:
        sess = session or SessionLocal()
        return TaskRepository(sess)
        
    @staticmethod
    def create_worklog_repository(session: Session = None) -> WorklogRepository:
        sess = session or SessionLocal()
        return WorklogRepository(sess)
