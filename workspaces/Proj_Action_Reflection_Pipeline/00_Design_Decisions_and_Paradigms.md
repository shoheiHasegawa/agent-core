...
*   **タスクの隠蔽管理**: 「10/15までに〇〇の資料を作る」といった期限付きの短期タスクは、すべて `agent-core/data/task_registry/` という秘書専用の領域に保存されます。CEOは毎日「今日やるべきことのブリーフィング」を見るだけで済みます。
...
## 5. DDDアーキテクチャの洗練（レガシーからの脱却）
旧来の `*UseCase` による過度なクラス細分化（タスク追加用クラス、パース用クラス等の乱立）を廃止し、業務フロー単位でオーケストレーションを行う `*Service.*()` パターン（例: `DailyPlanningService.generate_today_plan`）を採用しました。これにより、ドメインの意図が直感的に伝わる美しいコードベースを実現します。
