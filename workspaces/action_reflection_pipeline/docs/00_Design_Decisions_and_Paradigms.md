...
## 8. タスク種別とライフサイクルの境界ルール (One-off vs Recurring)

タスクの持ち越し（ロールオーバー）および消滅に関して、以下の境界ルールを厳格に適用します。

*   **一回限りタスク (One-off Tasks: `task_type == 'ONE_OFF'`)**:
    *   **ライフサイクル**: 完了（`COMPLETED`）するまで永続化される。
    *   **ロールオーバー挙動**: 指定日に未完了（`TODO` / `IN_PROGRESS`）だった場合、翌日の計画生成時に**自動的に翌日へ持ち越される**（`target_date = target_date`）。
*   **定期ルーティンタスク (Recurring Tasks: `task_type == 'RECURRING'` / `00_Recurring`)**:
    *   **ライフサイクル**: その日限りのインスタンス（日次生成）として生成される。
    *   **ロールオーバー挙動**: 指定日に未完了であっても**翌日へ持ち越してはならない（ロールオーバー抑止）**。休日のルーティン（例: 家事ローテーション）が平日に侵食することや、日課が累積してWIPを圧迫することを防ぐため、未消化分は当日限りの履歴として残し、翌日はその日のcronルールに従って新規判定・生成する。
