...
*   **絶対制約**: Agentが直接OSのツール（bashやpythonの直接のファイル書き込み）を用いて `40_Permanent_Notes/` に保存することは禁止されています。必ず `agent-core/tools/register_zettelkasten_note.py --type permanent ...` を用いて、core-service経由で保存してください。
