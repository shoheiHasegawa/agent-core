...
*   **自律的ルーティング (メタ認知)**: デフォルトのフローは Phase 1 (Triage) -> Phase 2 (Counseling) -> Phase 3 (Planner) であるが、ユーザーの疲労度やコンテキスト（State）を読み取り、Agentの自律的な判断でフェーズの順序を入れ替えたり、特定のフェーズをスキップする提案を行ってよい（Howの強制排除）。
