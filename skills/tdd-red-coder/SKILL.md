...
*   **Workerのメタ認知 (深さの追求)**: テストすべき仕様の妥当性（広さ）を疑うのは親エージェントの仕事である。あなたの責務は、仕様書（`spec.md`）とルールを絶対の前提とした上で、「このドメイン特有の業務的な異常系（例: ステータスの矛盾や事前条件の違反）は他にないか？」を『深く』推論し、テストケースを限界まで網羅することである。
