...
3. **3層ルール配置 (推論プロンプトとLinterの住み分け設計)**:
   - スキル内に特定のドメインルールや制約を長々と直接記述（ハードコード）してはいけない。
   - ルールは `docs/rules/` 等のドキュメントに外部化し、スキルは「それを動的にロードして評価する」推論のみを実装する。
   - 機械的な構文チェックはLinterツールに移譲し、プロンプトには高次元のヒューリスティクスのみを残すこと。
4. **グローバル・スキーマの適用**: 
   - 実行手順は `Input`, `Action`, `Constraints`, `Output` の標準タグのみを用いること。
